...
extension.filters = function (changed) {
	...
	if (
		...
		} else if (theme === 'reading' && extension.websiteHasDarkTheme === false) {
			// Reading Mode: Soft invert with sepia and reduced brightness
			style = 'invert(1) sepia(0.2) brightness(0.9) contrast(1.1)';

			extension.allowColors();

			html.setAttribute('dm-reading-mode', 'true');
		} else {
			html.removeAttribute('dm-invert-colors');
			html.removeAttribute('dm-reading-mode');
		...
		// Extension is turned off - clear all attributes and deactivate filters
		html.removeAttribute('dm-invert-colors');
		html.removeAttribute('dm-reading-mode');
		extension.allowColors();
		extension.dynamicFilter.deactivate();
		bluelight = 0; // Reset bluelight when extension is off
	}
